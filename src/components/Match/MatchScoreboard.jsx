...
// SetInput — a single score input cell
function SetInput({ value, onChange, disabled, isMe }) {
  return (
    <input
      ...
      disabled={disabled}
      className={clsx(
        "w-10 h-10 text-center text-sm font-semibold rounded-lg border outline-none",
        "disabled:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed",
        isMe
          ? "border-tennis-light focus:ring-2 focus:ring-tennis-light"
...
// WinsCircle — filled dark green when this player won more sets
...
export default function MatchScoreboard({ sets, onChange, myName, oppName }) {
  // sets: [{me: null|int, opp: null|int, tiebreak: null|int}, ...]
  const s1 = sets[0];
  const s2 = sets[1];
  const s3 = sets[2];
  ...
  function update(setIdx, field, value) {
    ...
    // Reset downstream sets when upstream changes
    if (setIdx === 0) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      {/* Header row */}
      <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-center px-4 py-2 bg-gray-50 border-b border-gray-100 text-xs text-gray-500 font-medium">
        <span>Player</span>
      ...
      {/* You row */}
      <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-center px-4 py-3 border-b border-gray-100">
        <span className="text-sm font-medium text-tennis-dark truncate">{myName ?? "You"}</span>
      ...
      <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-2 items-center px-4 py-3">
        <span className="text-sm text-gray-700 truncate">{oppName ?? "Opponent"}</span>
          ...
          {showTiebreak1 && (
            <label className="flex items-center gap-1">
              ...
              <input
                ...
                className="w-10 h-8 text-center text-sm border border-gray-200 rounded-lg"
              />
            </label>
          )}
          {showTiebreak2 && (
